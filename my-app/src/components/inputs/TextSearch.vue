<template>
  <button
    class="ft-button" 
    :class="{'big': big, 'secondary': priority === 'secondary'}"
  >
      <!-- @click="myFunction()" -->
    <slot />
    <!-- <p> {{ randomNumber }} </p> -->
...
export default {
  ...
  methods: {
    myFunction() {
      this.randomNumber = Math.floor(Math.random()*10) //multiplied to get random number 0 - 100
    }
  }
}
</script>
...
.ft-button {
  ...
  box-shadow: 0 10px rgb(70, 70, 70);
}
